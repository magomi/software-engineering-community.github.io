	...
	eventData.push(JSON.parse('{"status": "upcoming", "local_date": "2020-05-05", "local_time": "12:00", "link": "https://devday.de", "name": "DevDay 2020"}'));
        ...
        for (var index = 0; index < eventData.length; index++) {
            ...
            if (event.status === 'upcoming') {
                var eventDate = $.datepicker.formatDate('dd M', new Date(event.local_date));
                var template = '<li class="event"><a href="'+event.link+'"><span class="date"><span class="datestring">'+eventDate+'</span> <span class="time">'+event.local_time+'</span></span><span class="event-title">'+ event.name + '</span></a></li>';
